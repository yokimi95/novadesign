  ...
  if (odDataDir) {
    ...
    if (!isAbs) {
      throw new PackagedPathAccessError(
        [
          "Novago's packaged runtime requires OD_DATA_DIR to be an absolute path.",
          "",
          `Configured value: ${odDataDir}`,
          "",
          "Set OD_DATA_DIR to an absolute path (for example, C:\\\\Users\\\\You\\\\Novago on Windows or /Users/you/Novago on macOS/Linux) and relaunch Novago.",
        ].join("\n"),
        { title: "Novago cannot start with this OD_DATA_DIR" },
      );
    }
    ...
    if (scopedNamespace) {
      if (scopedNamespace !== namespace) {
        throw new PackagedPathAccessError(
          [
            "Novago's packaged runtime requires OD_DATA_DIR to target the active namespace.",
            "",
            `Configured value: ${odDataDir}`,
...
export function resolvePackagedNamespacePaths(
  ...
  // `~/Library/Application Support/Novago Prerelease/` on mac. Custom
